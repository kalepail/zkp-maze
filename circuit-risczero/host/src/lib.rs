...
/// * `Err` - If proof generation or verification fails
///
/// # Example
/// ```no_run
...
pub fn generate_maze_proof(
    ...
    // Verify the proof
    tracing::info!("Verifying maze proof...");
    receipt
        .verify(MAZE_GEN_ID)
        .map_err(|e| format!("Failed to verify maze proof: {}", e))?;
...
/// Verify a player's path through a proven maze (Hash-Based Architecture).
///
/// This creates a cryptographic proof that:
/// 1. The maze was correctly generated (by verifying maze_proof)
/// 2. The provided grid matches the committed hash
/// 3. The player's path successfully navigates from start to end
///
/// The final receipt proves all three conditions.
///
/// # Arguments
...
pub fn verify_path_proof(
    ...
    // Verify the proof
    tracing::info!("Verifying path proof...");
    receipt
        .verify(PATH_VERIFY_ID)
        .map_err(|e| format!("Failed to verify path proof: {}", e))?;
